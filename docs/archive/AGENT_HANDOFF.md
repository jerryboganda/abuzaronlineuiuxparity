...
> **Current status (2026-08-07):** the A–Z implementation slices and automated
> contracts are substantially complete; the latest post-change web suite passed
> 77/77 serial test cases, alongside focused no-retry checks for the base/child
> Change User and Window/MDI paths, DB-backed Go suites, Svelte check, and
> production build green. Overall
> acceptance remains **HOLD** because canonical historical reconciliation,
> exact legacy report/number proof, independent 1936x1048 raster pairs,
> physical-device/UAT sign-off, full-volume performance, and operational
> rollback evidence are still open. Do not claim 100% parity until the
> evidence gates in `docs/evidence/ACCEPTANCE_EVIDENCE_2026-08-07.md` are closed.

Read these three documents FIRST, in this order:
1. `docs/archive/GAP_ANALYSIS_2026-08-06.md` — the 15 verified gap areas (G1–G15) with evidence.
...
**Local dev credentials (never commit, publish, or copy into chat):**
- SQL Server and application credentials are stored in the local machine secret store/configuration. Load them through environment variables or the approved operator-only credential workflow; do not place passwords in commands, documentation, source, or logs.
- Legacy login: retrieve the approved operator credential from the local secret store; use only with the sandbox/reference application and database.
- New-app login: retrieve the approved development credential from the local secret store. If the local password must be reset, use the existing operator-only reset procedure with a secret supplied interactively, never as a literal in a script or document.
...
```
The local supervisor now launches the Vite dev server detached and restarts
failed children. If the supervisor is unavailable, start the web server
yourself as a **detached** process:
```powershell
...
```
- Retrieve the approved operator credential from the local secret store. It may already be running — check `Get-Process abuzar` first and reuse the instance.
...
- **B** Quick visual wins — source mojibake and the live username/titlebar clock are now covered by the current source gate and Phase B browser checks; remaining items are label normalization and distinct `Open Sale Return` kinds.
...
- **E** ⚠ **CRITICAL PATH — finish canonical data migration**: `migration/cmd/inspect` → author mapping waves in `migration/maps/` (masters → documents → ledgers → stock → security) → `import` with legacy-ID mappings → `reconcile` with business metrics. Reviewed master/security/pricing/tax/purchase/return slices and bounded historical waves exist, but canonical sales, full stock/GL/party-ledger reconciliation, residual source exceptions, and the complete source-table inventory remain open. See `migration/PHASE_E_HISTORICAL_STATUS_2026-08-06.md`.
- **F** Master-data engine: shared legacy list chrome (Sort/Filter/Find/List↔Detail), ~24 explicit master forms, Item master with **Suppliers sub-grid** (Priority/Rate/Disc%/Qty/Bonus/Days), and no demo-row fallback. Canonical masters use normalized APIs; the 16 auxiliary Basic Data leaves use an explicitly labelled tenant-scoped source-shaped CRUD compatibility API with confirmed delete until source-backed schemas and rules are approved.
- **G** Pricing/discount engine: the ten-level SalePrice selector now reprices the live grid and preserves the captured tier array; canonical purchases inherit valid tenant-scoped ItemSuppliers discount/bonus schemes; Manage now has a scoped imported `GroupAllowedPrice` editor. Exact customer/group enforcement, `PriceTypeCode`/`Module` mapping, PricePolicyDetail date semantics, complete policy promotion, ItemSuppliers day semantics, and the 50-invoice golden test remain open.
...
- **S** Maintenance real logic: `Lock Item Batches` now has a verified branch-scoped mutation; backup/restore, imports/exports (templates), stock adjustment/opening tools, integrity checks, and all 200+ preference behaviors remain open.
- **T** Manage module: Session Monitor (live sessions), user admin, imported group-scope setting editors, SMS/Email templates.
- **U** Hardware via edge service ("No adapter configured" only when no adapter is wired up — the plumbing itself is now code-complete and unit-tested for every category, see `docs/evidence/PHASE_U_HARDWARE_EVIDENCE.md`): ESC/POS thermal slip + label printing (byte-match legacy output), barcode wedge → item lookup (UI wiring landed 2026-08-08 in `apps/web/src/lib/barcode-scanner.ts`), cash-drawer kick, biometric (adapter interface + edge route exist; matching algorithm itself is unimplemented by design — no vendor SDK/hardware available), SMS gateway, SMTP (adapters + edge routes + central-API channel bridge landed 2026-08-08). All categories still require a real physical device or live gateway credentials and a signed operator run before acceptance.
...
**Real & working:** auth/session/tenancy (RLS-scoped), generic event-sourced document posting with idempotency + offline queue, normalized core masters plus tenant-scoped auxiliary master CRUD, 3 real report projections, menu shell from 275-entry catalog, raster-baseline shell (`differentPixels=0` on several captured defaults), migration tooling (unused), Tauri desktop build, local ops scripts.

**Façade (looks done, isn't):** most menu leaves route to generic surfaces; report leaves → generic 6-column grid; Maintenance/Manage forms persist to `tenant_preferences` and execute nothing; document status hardcoded `posted`; discounts read-only 0.00; no tax/stock/batch/GL/ledger/rights logic; item lookup falls back to a hardcoded 32-item list; title bar shows a frozen captured timestamp; mojibake in source masked by a runtime MutationObserver. Hardware adapters return "No adapter configured" only when unconfigured — as of 2026-08-08 the adapter/routing/registry code for printer, barcode (incl. UI wiring), cash drawer, biometric, SMS, and SMTP all exists and is unit-tested (see `docs/evidence/PHASE_U_HARDWARE_EVIDENCE.md`); no physical device or live gateway credential has been exercised, and biometric matching has no implementation by design.
