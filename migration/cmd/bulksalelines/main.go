// Command bulksalelines imports the reviewed canonical Saledetail slice with
// PostgreSQL COPY and set-based dependency joins. SQL Server remains
...
// Keep the source expression aligned with migration/maps/phase-e-historical-
// documents.json. The quantity expression is deliberately not rounded so the
// target retains the legacy pack/loose-unit calculation before numeric scale.
const sourceQuery = `
SELECT
  CONVERT(varchar(100), d.SaleInvcode) AS sale_inv_code,
  CONVERT(varchar(30), d.RowID) AS sale_row_id,
...
func main() {
	...
	out := flag.String("out", filepath.Join("parity", "catalog", "canonical-first-tenant-sale-lines-import.json"), "report path")
	flag.Parse()
	...
	if _, err := tx.Exec(ctx, `CREATE TEMP TABLE sale_line_stage (
		...
		fatal(fmt.Sprintf("create sale-line staging table: %v", err))
	}
	copyRows := make([][]any, 0, len(rows))
	for _, row := range rows {
		copyRows = append(copyRows, []any{
			row.SaleID, row.RowID, row.LegacyID, row.LegacyKey,
	...
	if len(copyRows) > 0 {
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"sale_line_stage"}, []string{
			"sale_id", "row_id", "legacy_id", "legacy_import_key", "item_legacy_id", "item_code",
			...
			fatal(fmt.Sprintf("copy sale-line staging rows: %v", err))
		}
	}

	missing, err := dependencyExceptions(ctx, tx, *tenant, *branch)
	if err != nil {
		fatal(fmt.Sprintf("check sale-line dependencies: %v", err))
	}
	...
	if len(invalid) > 0 {
		if err := writeExceptions(ctx, tx, *tenant, invalid); err != nil {
			fatal(fmt.Sprintf("record invalid sale-line mappings: %v", err))
		}
	}
	if len(missing) > 0 {
	...
	var before int
	if err := tx.QueryRow(ctx, `SELECT COUNT(*) FROM business_document_lines l JOIN sale_line_stage s ON s.legacy_import_key = l.legacy_import_key WHERE l.tenant_id = $1 AND l.branch_id = $2`, *tenant, *branch).Scan(&before); err != nil {
		fatal(fmt.Sprintf("count existing sale lines: %v", err))
	}
	...
	SELECT $1, $2, d.id, s.row_id, i.id, s.item_legacy_id, s.item_code, i.name,
		...
		s.gst_rate, s.advance_tax_rate, s.tax_amount, 'Saledetail', s.legacy_id, s.payload, s.legacy_import_key
	FROM sale_line_stage s
	JOIN business_documents d
	  ON d.tenant_id = $1 AND d.branch_id = $2 AND d.legacy_source_table = 'SaleLedger' AND d.legacy_id = s.sale_id
	JOIN master_items i
	...
	ON CONFLICT (tenant_id, branch_id, legacy_import_key) DO UPDATE SET
		...
		legacy_payload = EXCLUDED.legacy_payload`, *tenant, *branch); err != nil {
		fatal(fmt.Sprintf("upsert sale lines: %v", err))
	}
	var after int
	if err := tx.QueryRow(ctx, `SELECT COUNT(*) FROM business_document_lines l JOIN sale_line_stage s ON s.legacy_import_key = l.legacy_import_key WHERE l.tenant_id = $1 AND l.branch_id = $2`, *tenant, *branch).Scan(&after); err != nil {
		fatal(fmt.Sprintf("count imported sale lines: %v", err))
	}
	result.Imported = after - before
	result.Duplicates += before
	if err := writeMappedMappings(ctx, tx, *tenant); err != nil {
		fatal(fmt.Sprintf("record sale-line mappings: %v", err))
	}
	if err := tx.Commit(ctx); err != nil {
		fatal(fmt.Sprintf("commit sale lines: %v", err))
	}
	...
	fmt.Printf("Bulk processed sale-line rows %d-%s for tenant %s; read %d, imported %d, exceptions %d; report: %s\n", *fromRow, rowWindowEnd(*toRow), *tenant, result.Read, result.Imported, result.Exceptions, *out)
}
...
func readRows(ctx context.Context, source *sql.DB, fromRow, toRow int) ([]sourceRow, int, []exceptionRow, error) {
	...
	if err != nil {
		return nil, 0, nil, fmt.Errorf("read dbo.Saledetail: %w", err)
	}
	defer rows.Close()
	result := make([]sourceRow, 0, 620000)
	seen := make(map[string]struct{}, 620000)
	duplicates := 0
	...
	for rows.Next() {
		...
		if err := rows.Scan(pointers...); err != nil {
			return nil, 0, nil, fmt.Errorf("scan Saledetail row %d: %w", len(result)+len(exceptions)+duplicates+1, err)
		}
		saleID := normalizeText(values[0])
		rowText := normalizeText(values[1])
		itemID := normalizeText(values[2])
		legacyID := saleID + ":" + rowText
		legacyKey := "Saledetail:" + legacyID
		...
		if reason != "" {
			details, err := saleLineExceptionDetails(values, legacyID, reason)
			if err != nil {
				return nil, 0, nil, fmt.Errorf("encode Saledetail exception %s: %w", legacyID, err)
			}
			exceptions = append(exceptions, exceptionRow{LegacyID: legacyID, Reason: reason, Details: details})
			continue
		}
		payload, err := json.Marshal(map[string]any{
			"SaleInvcode":  normalizeValue(values[0]),
			...
			"SalePrice":    normalizeValue(values[17]),
			"Rate":         normalizeValue(values[18]),
			"DiscPerc":     normalizeValue(values[19]),
			"SalesTax":     normalizeValue(values[20]),
			"itemflatdisc": normalizeValue(values[21]),
			"RowID":        normalizeValue(values[1]),
			"HSCode":       normalizeValue(values[22]),
			"UOM":          normalizeValue(values[23]),
		})
		if err != nil {
			return nil, 0, nil, fmt.Errorf("encode Saledetail row %s: %w", legacyID, err)
		}
		expiry := normalizeDate(values[12])
		result = append(result, sourceRow{
			LegacyID: legacyID, LegacyKey: legacyKey, SaleID: saleID, RowID: rowID,
			...
			ExpiryDate: expiry, Payload: payload,
		})
	}
	if err := rows.Err(); err != nil {
		return nil, 0, nil, fmt.Errorf("read Saledetail rows: %w", err)
	}
...
ORDER BY sale_inv_code,
         TRY_CONVERT(bigint, sale_row_id),
         sale_row_id,
         item_legacy_id
OFFSET ? ROWS`
	args := []any{fromRow}
...
func dependencyExceptions(ctx context.Context, tx pgx.Tx, tenant, branch string) ([]exceptionRow, error) {
	rows, err := tx.Query(ctx, `SELECT s.legacy_id,
		...
		FROM sale_line_stage s
		LEFT JOIN business_documents d ON d.tenant_id = $1 AND d.branch_id = $2 AND d.legacy_source_table = 'SaleLedger' AND d.legacy_id = s.sale_id
...
func writeExceptions(ctx context.Context, tx pgx.Tx, tenant string, items []exceptionRow) error {
	...
	if _, err := tx.Exec(ctx, `CREATE TEMP TABLE IF NOT EXISTS sale_line_exception_stage (
		legacy_id text NOT NULL,
	...
	if _, err := tx.Exec(ctx, `TRUNCATE sale_line_exception_stage`); err != nil {
		return err
	}
	...
	if _, err := tx.CopyFrom(ctx, pgx.Identifier{"sale_line_exception_stage"}, []string{"legacy_id", "reason", "details"}, pgx.CopyFromRows(rows)); err != nil {
		return err
	}
	...
	SELECT $1, 'sqlserver', 'dbo', 'Saledetail', legacy_id, $2, NULL, 'exception', reason
	FROM sale_line_exception_stage
	...
	if _, err := tx.Exec(ctx, `DELETE FROM migration_exceptions
		WHERE tenant_id = $1 AND source_schema = 'dbo' AND source_table = 'Saledetail'
		  AND legacy_id IN (SELECT legacy_id FROM sale_line_exception_stage)`, tenant); err != nil {
		return err
	}
	_, err := tx.Exec(ctx, `INSERT INTO migration_exceptions (
		...
		SELECT $1, 'dbo', 'Saledetail', legacy_id, reason,
			details, 'open'
		FROM sale_line_exception_stage`, tenant)
	return err
}
...
func writeMappedMappings(ctx context.Context, tx pgx.Tx, tenant string) error {
	...
	SELECT $1, 'sqlserver', 'dbo', 'Saledetail', s.legacy_id, $2, l.id::text, 'mapped', NULL
	FROM sale_line_stage s
...
func saleLineExceptionDetails(values []any, legacyID, reason string) ([]byte, error) {
	if len(values) < 24 {
		return nil, fmt.Errorf("Saledetail row has %d values; want at least 24", len(values))
	}
	return json.Marshal(map[string]any{
		"legacy_id": legacyID, "reason": reason,
		"SaleInvcode": normalizeValue(values[0]), "RowID": normalizeValue(values[1]),
		...
		"PackUnits": normalizeValue(values[16]), "SalePrice": normalizeValue(values[17]),
		"Rate": normalizeValue(values[18]), "DiscPerc": normalizeValue(values[19]),
		"SalesTax": normalizeValue(values[20]), "itemflatdisc": normalizeValue(values[21]),
		"HSCode": normalizeValue(values[22]), "UOM": normalizeValue(values[23]),
	})
}
