// Command bulkpurchaselines imports the reviewed canonical Purdetail slice
// with PostgreSQL COPY and set-based dependency joins. The generic importer is
// deliberately row-oriented for heterogeneous maps; this bounded command is
// used for the large, stable purchase-line table so a restart remains fast and
// deterministic while the SQL Server source stays read-only.
package main
...
const sourceQuery = `
SELECT
  CONVERT(varchar(100), d.PurInvCode) AS pur_inv_code,
  CONVERT(varchar(30), d.PurRowId) AS pur_row_id,
  ...
  d.UnitSalesTax,
  d.PCTCode,
  d.SalesTaxScheduleCode,
  d.AvgPrice,
  d.BonusQty,
  d.ItemAdvanceTaxPerc
FROM dbo.Purdetail AS d
CROSS APPLY (
...
func main() {
	...
	out := flag.String("out", filepath.Join("parity", "catalog", "canonical-first-tenant-purchase-lines-import.json"), "report path")
	flag.Parse()
	...
	if _, err := tx.Exec(ctx, `CREATE TEMP TABLE purchase_line_stage (
		...
		fatal(fmt.Sprintf("create purchase-line staging table: %v", err))
	}
	copyRows := make([][]any, 0, len(rows))
	for _, row := range rows {
		copyRows = append(copyRows, []any{
			row.PurchaseID, row.RowID, row.LegacyID, row.LegacyKey,
	...
	if len(copyRows) > 0 {
		if _, err := tx.CopyFrom(ctx, pgx.Identifier{"purchase_line_stage"}, []string{
			"purchase_id", "row_id", "legacy_id", "legacy_import_key", "item_legacy_id", "item_code",
			...
			fatal(fmt.Sprintf("copy purchase-line staging rows: %v", err))
		}
	}

	missing, err := dependencyExceptions(ctx, tx, *tenant, *branch)
	if err != nil {
		fatal(fmt.Sprintf("check purchase-line dependencies: %v", err))
	}
	...
	if len(invalid) > 0 {
		if err := writeExceptions(ctx, tx, *tenant, invalid); err != nil {
			fatal(fmt.Sprintf("record invalid purchase-line mappings: %v", err))
		}
	}
	if len(missing) > 0 {
	...
	var before int
	if err := tx.QueryRow(ctx, `SELECT COUNT(*) FROM business_document_lines l JOIN purchase_line_stage s ON s.legacy_import_key = l.legacy_import_key WHERE l.tenant_id = $1 AND l.branch_id = $2`, *tenant, *branch).Scan(&before); err != nil {
		fatal(fmt.Sprintf("count existing purchase lines: %v", err))
	}
	...
	SELECT $1, $2, d.id, s.row_id, i.id, s.item_legacy_id, s.item_code, i.name,
		...
		s.gst_rate, s.advance_tax_rate, s.tax_amount, 'Purdetail', s.legacy_id, s.payload, s.legacy_import_key
	FROM purchase_line_stage s
	JOIN business_documents d
	  ON d.tenant_id = $1 AND d.branch_id = $2 AND d.legacy_source_table = 'Purledger' AND d.legacy_id = s.purchase_id
	JOIN master_items i
	...
	ON CONFLICT (tenant_id, branch_id, legacy_import_key) DO UPDATE SET
		...
		fatal(fmt.Sprintf("upsert purchase lines: %v", err))
	}
	var after int
	if err := tx.QueryRow(ctx, `SELECT COUNT(*) FROM business_document_lines l JOIN purchase_line_stage s ON s.legacy_import_key = l.legacy_import_key WHERE l.tenant_id = $1 AND l.branch_id = $2`, *tenant, *branch).Scan(&after); err != nil {
		fatal(fmt.Sprintf("count imported purchase lines: %v", err))
	}
	result.Imported = after - before
	result.Duplicates += before
	if err := writeMappedMappings(ctx, tx, *tenant); err != nil {
		fatal(fmt.Sprintf("record purchase-line mappings: %v", err))
	}
	if err := tx.Commit(ctx); err != nil {
		fatal(fmt.Sprintf("commit purchase lines: %v", err))
	}
	...
	fmt.Printf("Bulk processed purchase-line rows %d-%s for tenant %s; read %d, imported %d, exceptions %d; report: %s\n", *fromRow, rowWindowEnd(*toRow), *tenant, result.Read, result.Imported, result.Exceptions, *out)
}
...
func readRows(ctx context.Context, source *sql.DB, fromRow, toRow int) ([]sourceRow, int, []exceptionRow, error) {
	...
	if err != nil {
		return nil, 0, nil, fmt.Errorf("read dbo.Purdetail: %w", err)
	}
	defer rows.Close()
	result := make([]sourceRow, 0, 120000)
	seen := make(map[string]struct{}, 120000)
	duplicates := 0
	...
	for rows.Next() {
		...
		if err := rows.Scan(pointers...); err != nil {
			return nil, 0, nil, fmt.Errorf("scan Purdetail row %d: %w", len(result)+len(exceptions)+duplicates+1, err)
		}
		purchaseID := normalizeText(values[0])
		rowText := normalizeText(values[1])
		itemID := normalizeText(values[2])
		legacyID := purchaseID + ":" + rowText
		legacyKey := "Purdetail:" + legacyID
		...
		if err != nil {
			return nil, 0, nil, fmt.Errorf("encode Purdetail row %s: %w", legacyID, err)
		}
		expiry := normalizeDate(values[12])
		result = append(result, sourceRow{
			LegacyID: legacyID, LegacyKey: legacyKey, PurchaseID: purchaseID, RowID: rowID,
	...
	if err := rows.Err(); err != nil {
		return nil, 0, nil, fmt.Errorf("read Purdetail rows: %w", err)
	}
...
ORDER BY pur_inv_code,
         TRY_CONVERT(bigint, pur_row_id),
         pur_row_id,
         item_legacy_id
OFFSET ? ROWS`
	args := []any{fromRow}
...
func dependencyExceptions(ctx context.Context, tx pgx.Tx, tenant, branch string) ([]exceptionRow, error) {
	rows, err := tx.Query(ctx, `SELECT s.legacy_id,
		...
		FROM purchase_line_stage s
		LEFT JOIN business_documents d ON d.tenant_id = $1 AND d.branch_id = $2 AND d.legacy_source_table = 'Purledger' AND d.legacy_id = s.purchase_id
...
func writeExceptions(ctx context.Context, tx pgx.Tx, tenant string, items []exceptionRow) error {
	...
	if _, err := tx.Exec(ctx, `TRUNCATE purchase_line_exception_stage`); err != nil {
		return err
	}
	...
	SELECT $1, 'sqlserver', 'dbo', 'Purdetail', legacy_id, $2, NULL, 'exception', reason
	FROM purchase_line_exception_stage
	ON CONFLICT (tenant_id, source_system, source_schema, source_table, legacy_id)
		DO UPDATE SET target_table = EXCLUDED.target_table, target_id = NULL, status = 'exception', note = EXCLUDED.note`, tenant, targetTable)
	if err != nil {
		return err
	}
	if _, err := tx.Exec(ctx, `DELETE FROM migration_exceptions
		WHERE tenant_id = $1 AND source_schema = 'dbo' AND source_table = 'Purdetail'
		  AND legacy_id IN (SELECT legacy_id FROM purchase_line_exception_stage)`, tenant); err != nil {
		return err
	}
...
func writeMappedMappings(ctx context.Context, tx pgx.Tx, tenant string) error {
	...
	SELECT $1, 'sqlserver', 'dbo', 'Purdetail', s.legacy_id, $2, l.id::text, 'mapped', NULL
	FROM purchase_line_stage s
