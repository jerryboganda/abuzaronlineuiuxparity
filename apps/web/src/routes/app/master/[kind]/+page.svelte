<script lang="ts">
  ...
  const definitions: Record<string, { title: string; fields: Field[] }> = {
    ...
    'customer-group': { title: 'Customer Group', fields: [
      { label: 'Code' }, { label: 'Name' },
      { label: 'Category', key: 'Category' },
      { label: 'Allowed Price #', key: 'GroupAllowedPrice' },
      { label: 'Credit Limit', key: 'CreditLimit', kind: 'number' },
      { label: 'Default Disc(%)', key: 'DefaultItemDiscPerc', kind: 'number' },
      { label: 'Description', kind: 'textarea' },
      { label: 'Active', kind: 'select', options: ['YES', 'NO'] }
    ] },
  ...
  let basicItemClass = '';
  let basicItemPacking = '';
  let basicAssociatedGodown = '';
  let allowDueDefaultValue = false;
  let salesPersonScope = '';
  ...
  onMount(() => {
    ...
    void api.preferences('BasicData').then((prefs) => {
      ...
      for (const item of prefs.registry ?? prefs.items ?? []) {
        ...
        if (item.caption === 'Item Packing:') basicItemPacking = item.value || basicItemPacking;
        if (item.caption === 'Associated Godown:') basicAssociatedGodown = item.value || basicAssociatedGodown;
        if (item.caption === 'Allow Due Default Value in Item Basic Data:') allowDueDefaultValue = yes(item.value);
        if (item.caption === 'Sales Person Scope For Sub Area:') salesPersonScope = item.value || salesPersonScope;
      }
    }).catch(() => {
...
<main class:legacy-master-list-tab={activeTab === 'list'} class:legacy-master-customer-baseline={kind === 'customer' && activeTab === 'detail' && !interactive} class:legacy-master-supplier-baseline={kind === 'supplier' && activeTab === 'detail' && !interactive} class:legacy-master-item-baseline={kind === 'item' && activeTab === 'detail' && !interactive} class:legacy-master-user-baseline={kind === 'user' && activeTab === 'detail' && !interactive} class="legacy-master-page" onpointerdown={enableInteractive} onfocusin={enableInteractive}><section class="legacy-master-window" aria-label={definition.title}>
  ...
  <div class="legacy-master-body"><form class="legacy-master-form" onsubmit={(event) => { event.preventDefault(); void saveRecord(); }}>
    ...
    {#if kind === 'item'}
      <label class="legacy-sale-optional-field">Item Packing:<input aria-label="Item packing" bind:value={basicItemPacking} /></label>
      <label class="legacy-sale-optional-field">Associated Godown:<input aria-label="Associated godown" bind:value={basicAssociatedGodown} /></label>
      {#if allowDueDefaultValue}<label class="legacy-sale-optional-field">Allow Due Default Value in Item Basic Data:<input type="checkbox" checked={allowDueDefaultValue} disabled /></label>{/if}
      <label class="legacy-sale-optional-field">Sales Person Scope For Sub Area:<input aria-label="Sales person scope for sub area" bind:value={salesPersonScope} /></label>
    {/if}
