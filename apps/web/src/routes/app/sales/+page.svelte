    ...
    void (async () => {
      try {
        ...
        try {
          ...
          try {
            const othersPrefs = await api.preferences('Others');
            for (const item of othersPrefs.registry ?? othersPrefs.items ?? []) {
              if (item.caption === 'Retail Price(%) for Sale Inv.:') showRetailPriceOnSaleInv = preferenceYes(item.value);
              if (item.caption === 'Mfg. Distributor Code:') mfgDistributorCode = item.value || mfgDistributorCode;
              if (item.caption === 'Distributor Code:') distributorCode = item.value || distributorCode;
            }
          } catch {
