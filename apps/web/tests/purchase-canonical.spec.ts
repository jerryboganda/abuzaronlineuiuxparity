...
async function fireBarcodeScan(page: Page, ariaLabel: string, barcode: string) {
...
test('scanning a barcode resolves it through the edge lookup route and adds a purchase line', async ({ page }) => {
  await mockCanonicalContext(page, false);
  ...
  await page.route(`**/v1/master/item/${itemId}`, (route) => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ id: itemId, kind: 'item', legacyId: 'ITEM-1', code: 'ITEM-1', name: 'Scanned Item', payload: { PurchasePrice: '4.50' }, active: true })
  }));
  await page.goto('/app/purchase/pack');
  await waitForPurchaseReady(page);
  await fireBarcodeScan(page, 'Item lookup query', '8901234567890');
  await expect.poll(() => lookupRaw).toBe('8901234567890');
  await expect(page.getByRole('combobox', { name: 'Item name 1' })).toHaveValue('Scanned Item');
