...
You are Worker 1 for Milestone M1 (Legacy Shell, Workflow & MDI Parity).
Working directory: d:\ABUZAR\AbuzarNext\.agents\worker_m1_1
...
3. Fix Go backend API (`services/api/internal/httpapi/`):
   a. Register `POST /v1/auth/change-user` endpoint in Go server routes to satisfy interface contract in PROJECT.md and SCOPE.md. Implement `s.changeUser` endpoint handler properly.
...
6. Write findings, changes, build/test results, and 5-component handoff report to `handoff.md` in `d:\ABUZAR\AbuzarNext\.agents\worker_m1_1`.
